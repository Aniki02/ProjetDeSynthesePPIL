...
int main()
{
	...
	cout << "------ Test Sauvegarder une Forme dans un fichier ---------" << endl;


	ofstream fichierEcriture("test.txt", ios::out | ios::trunc);
	if (fichierEcriture){
		Forme * c = new Cercle(Couleur(0,100,255), Vecteur2D(100,100), 800);
		SauverForme::sauver(fichierEcriture, c);
	...
	ifstream fichierLecture("test.txt", ios::in);
	if (fichierLecture){
		...
		vector<Forme *> formes = ChargeurListeFormes::charge(fichierLecture, sC);
